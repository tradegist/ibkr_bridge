...
- **`required_env`** — `DO_API_TOKEN`, `TWS_USERID`, `TWS_PASSWORD`, `VNC_SERVER_PASSWORD`, `API_TOKEN`, `VNC_DOMAIN`, `SITE_DOMAIN`. Currently enforced by standalone deploy; do not assume `sync` or shared-mode deploy will block when a required var is missing or empty unless that validation is added there too.
