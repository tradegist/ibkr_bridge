...
# must be redacted. Plain config keys (RELAYS, TRADING_MODE, ...) do NOT
# match — their values stay readable in the prompt so claude has the context
# it needs to judge service health.
...
def _collect_secrets_to_redact():
    ...
    ``TRADING_MODE=paper`` or ``RELAYS=ibkr`` is intentionally NOT redacted —
    those values are diagnostically useful and not credentials.
    """
    cfg = config()
    secrets = set()
    for name in (".env", ".env.droplet"):
...
def _fetch_droplet_state(droplet_ip):
...
def run_sanity_check(droplet_ip):
...
def post_deploy_sanity_check(droplet_ip, *, skip_flag):
