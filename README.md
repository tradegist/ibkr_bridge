...
- **Server-side execId dedupe** — the bridge tracks broadcast `execId`s per session and emits each fill at most once. Live `commissionReportEvent` broadcasts populate the dedupe set, so the reconcile path skips fills already emitted live. The set is cleared on reconnect (each daily session is fresh).
