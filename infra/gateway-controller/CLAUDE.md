...
The container mounts the host Docker socket (`/var/run/docker.sock`). Caddy rewrites `/gateway/*` → `/cgi-bin/*` before proxying. Exposed at `https://{VNC_DOMAIN}/gateway/*` and protected by the VNC-domain HTTP Basic Auth (see `infra/CLAUDE.md`).
