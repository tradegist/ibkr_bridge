...
- Set `DROPLET_IP`, `SSH_KEY`, and `SHARED_NETWORK` in `.env` (no `DO_API_TOKEN` needed). `SHARED_NETWORK` is **required** in shared mode — the CLI fails fast with a clear error if it's unset.
